...
| Cryptographic log integrity | ✅ Phase 3 — SHA-256 hash-chained `activity_logs`/`finance_audit_logs` (`prev_hash`/`row_hash`, per-tenant), admin-only `verify_*_chain()` RPCs, tamper detection proven via a dedicated test; ✅ Phase 7 — the same technique extended to released-document snapshots (`document_release_chain`), with a public, per-document verification badge on `/verify/:id` | `backend/supabase/migrations/0030_audit_log_hash_chain.sql`, `0034_document_release_chain.sql`, `backend/supabase/verify/02_seed_and_assertions.sql` |
...
| **Detect** | ✅ Phase 3 — hash-chained audit logs make tampering detectable, not just logged; ✅ Phase 7 — same for released documents | `backend/supabase/migrations/0030_audit_log_hash_chain.sql`, `0034_document_release_chain.sql` |
