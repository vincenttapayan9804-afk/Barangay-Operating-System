...
- **Biometric step-up (Security Phase 6)** — every role (admin/staff/viewer, not just MFA-required accounts) is subject to a CompreFace face-verification step-up once an account hits 3 failed sign-in attempts: `backend/supabase/functions/login-gate` proxies the GoTrue password grant server-side so the failure counter (`login_attempts`) is authoritative, not client-reported, and the *next* login — correct password or not — requires a face match against the account's enrolled template (`face_enrollments`, `frontend/src/features/settings/FaceEnrollmentSettings.tsx`) before a session is minted. An account with no face enrolled fails closed (soft-locked, HTTP 423) rather than silently skipping the check; an admin clears the lock from Settings' "Locked Accounts" panel (`login_attempts` RLS scopes this to same-barangay admins only). See `docs/DEPLOYMENT.md` "Biometric Step-Up Authentication (CompreFace)".
...
- **Rate limiting** — configurable via GoTrue's own rate-limit environment variables, and via Kong plugins in front of it. `/functions/v1/*` (where Phase 6's `login-gate` now also proxies password-grant traffic) already carried its own Kong rate-limiting plugin (`limit_by: ip`) before this phase, same protection GoTrue's own `/auth/v1/*` route has.
...
- **Document tamper-evidence (Security Phase 7)** — applies Phase 3's SHA-256 hash-chain technique to released documents: the moment a document transitions to `released`, a database trigger snapshots its public-facing fields (type, resident name, queue number, release date/recipient) into `document_release_chain`, chained to the previous release in the same barangay (`backend/supabase/migrations/0034_document_release_chain.sql`). The public `/verify/:id` page (`frontend/src/pages/VerifyDocumentPage.tsx`) recomputes that one document's hash live and shows a "tamper-evident, chain verified" badge — or a tamper warning if the stored snapshot no longer matches what's in `document_requests`. Admins get a stronger, whole-chain check via `verify_document_release_chain()`. Same limitation as Phase 3's chain: this detects modification of a released record or a forged/reordered release event, not outright deletion of the most recent release(s) from the chain (tail truncation) — an external anchor would be needed to close that gap, which is out of scope here.
